...
export default function ItemSearch(props) {
    ...
    return <>
        ...
        <div className="row"><div className="col-12"><div className="card"><div className="card-header"><strong className="float-left">{state.criteria.search == null && Object.keys(state.criteria.filters).length === 0 ? "Recently Modified" : "Search Results"}</strong><span className="float-right clickable"><a href={addHref} aria-label="Add item"><i className="fas fa-plus" aria-hidden="true" /></a></span></div><div className="table-responsive"><table className="table table-sm table-md table-hover table-striped table-bordered mb-0 glass-banded-table"><thead className="thead-dark"><tr>{state.metadata.statInfo.filter(visible).map(stat => <th key={stat.short} className="text-center" title={stat.display} noWrap=""><button type="button" className="item-sort-button" aria-label={`Sort by ${stat.display}`} onClick={() => runSearch({...state.criteria, sortBy: stat.var, sortAsc: state.criteria.sortBy === stat.var ? !state.criteria.sortAsc : false, page: 1})}>{stat.short}&nbsp;{state.criteria.sortBy === stat.var ? <i className={`fas fa-sort-${state.criteria.sortAsc ? "up" : "down"}`} aria-hidden="true" /> : !state.criteria.sortBy && <i className="fas fa-sort" aria-hidden="true" />}</button></th>)}</tr></thead><tbody>{state.results.map((item, index) => <tr key={item.id} className={glassTableBandClass(index)} onClick={event => { if (!event.target.closest("a, button")) window.location.href = `/items/details.html?id=${item.id}`; }} style={{cursor: "pointer"}}>{state.metadata.statInfo.filter(visible).map(stat => <td key={stat.short} className={stat.var === "name" ? "text-primary font-weight-bold text-nowrap" : stat.var === "alignRestriction" ? "text-center text-monospace" : "text-center"}>{stat.var === "name" ? <ItemResultName item={item} /> : resultValue(item, stat, state.metadata.constants)}</td>)}</tr>)}</tbody></table></div></div></div></div><br />
