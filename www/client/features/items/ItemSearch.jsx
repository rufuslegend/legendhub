...
export default function ItemSearch(props) {
    ...
    const visible = stat => stat.var === "name" || state.selectedColumns.includes(stat.short);
    ...
    return <>
        ...
        <ColumnsDialog categories={state.metadata.categories} open={columnsOpen} onClose={closeColumns} onToggle={short => { const columns = state.selectedColumns.includes(short) ? state.selectedColumns.filter(value => value !== short) : [...state.selectedColumns, short]; persistColumns(columns); dispatch({type: "column/toggle", short}); }} onReset={() => { const columns = state.metadata.statInfo.filter(stat => stat.showColumnDefault || stat.var === "name").map(stat => stat.short); persistColumns(columns); dispatch({type: "column/reset"}); }} requiredColumns={state.metadata.statInfo.filter(stat => stat.var === "name").map(stat => stat.short)} selectedColumns={state.selectedColumns} triggerRef={columnsTriggerRef} />
