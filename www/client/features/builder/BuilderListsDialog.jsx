...
export default function BuilderListsDialog({dialog, state, onClose, onSubmit}) {
    ...
    return <BuilderModal label={title} onClose={onClose} initialFocus="#builder-list-name"><form className="modal-body" noValidate onSubmit={event => { event.preventDefault(); onSubmit(name); }}><label htmlFor="builder-list-name">Name</label><input id="builder-list-name" className="form-control" pattern="[A-Za-z\\s\\d]+" value={name} aria-invalid={Boolean(state.dialogError)} aria-describedby={state.dialogError ? errorId : undefined} onChange={event => onSubmit(event.target.value, true)} />{state.dialogError && <p id={errorId} role="alert" className="text-danger mb-0">{state.dialogError}</p>}<button className="btn btn-primary mt-3" type="submit">{dialog === "add-character" ? "Add" : "Save"}</button></form></BuilderModal>;
}
