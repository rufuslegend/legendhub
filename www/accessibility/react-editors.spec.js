...
test("React migration: item editor preserves fields, independent required validation, lookups, and add/edit saves", async function({page}) {
    ...
    await expect(closeLookup).toBeFocused();
    const itemName = page.locator('input[name="name"]');
    expect(await itemName.evaluate(element => element.closest("[inert]") != null)).toBe(true);
    await itemName.focus();
    await expect(closeLookup).toBeFocused();
    const itemNameBounds = await itemName.boundingBox();
    expect(itemNameBounds).not.toBeNull();
    await page.mouse.click(itemNameBounds.x + 5, itemNameBounds.y + 5);
    await expect(itemName).not.toBeFocused();
