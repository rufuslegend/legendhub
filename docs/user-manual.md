...
When the **Mit** column is visible, both Total rows show mitigation with **Cap:** underneath it. Normal equipment and natural Battle Training mitigation are capped first; bonuses from affects are added afterward, so the final value can exceed the displayed cap. For example, `23` from equipment plus `11` from Battle Training is capped at `30`, then `5` from affects gives `35`.

An Other entry representing a skill can carry both mitigation and a matching cap bonus. Adding **MitCap +2** to a Bastion entry that already has **Mitigation +2** raises this example's displayed cap to `32` while mitigation stays `35`. The matching positive cap bonus provides room for the skill's existing mitigation; it does not also release extra equipment mitigation. Any cap bonus beyond that entry's positive mitigation, a cap-only bonus, and negative cap modifiers still adjust the equipment-and-training limit. For example, an additional cap-only `+5 MitCap` raises the cap to `35`, allowing all `34` equipment-and-training mitigation plus `5` from affects, for `39`.

The cap warning reports equipment plus training before its limit, excluding affects; this limit can be lower than the displayed cap when skills have matching cap bonuses. The Builder does not automatically infer Chant/axiom or Rage penalties; model them with the appropriate negative MitCap on an Other entry. Battle Training's automatic `+10` cap allowance is already included, so do not enter that allowance a second time as an item modifier.
